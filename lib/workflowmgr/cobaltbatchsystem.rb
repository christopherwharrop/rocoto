  ...
  class COBALTBatchSystem < BatchSystem
    ...
    def submit(task)
      ...
      task.attributes.each do |option, value|
        ...
        when :partition
          ...
          WorkflowMgr.log("WARNING: the <partition> tag is not supported for Cobalt.")
        when :cores
      ...
      WorkflowMgr.stderr(
        "Submitting #{task.attributes[:name]} using #{cmd} --mode script #{tf.path} with input {{#{input}}}", 4
      )
      ...
      output = if WorkflowMgr.dryrun_mode?
                 ...
                 `#{cmd} --mode script #{tf.path} 2>&1`.chomp
               end
    ...
    def refresh_jobqueue
      ...
      queued_jobs.split(/\n/).each do |job|
        ...
        when /State\s+:\s+(\S+)/
          record[:native_state] = ::Regexp.last_match(1)
        ...
        when /JobName\s+:\s+(\S+)/
          record[:jobname] = ::Regexp.last_match(1)
        when /User\s+:\s+(\S+)/
          record[:user] = ::Regexp.last_match(1)
        when /Procs\s+:\s+(\d+)/
          record[:cores] = ::Regexp.last_match(1).to_i
        when /Queue\s+:\s+(\S+)/
          record[:queue] = ::Regexp.last_match(1)
        when /SubmitTime\s+:\s+(\S+.*)/
          record[:submit_time] = Time.gm(*ParseDate.parsedate(::Regexp.last_match(1)))
        when /StartTime\s+:\s+(\S+.*)/
          unless ::Regexp.last_match(1) == "N/A"
            record[:start_time] =
              Time.gm(*ParseDate.parsedate(::Regexp.last_match(1)))
          end
        when /(\S+)\s+:\s+(\S+)/
    ...
    def refresh_jobacct(jobid)
      ...
      joblog.split(/\n/).each do |line|
        ...
        when /task completed normally with an exit code of (\d+);/
          ...
          record[:exit_status] = ::Regexp.last_match(1).to_i
        when /^qsub.* (\S+)$/
          record[:command] = ::Regexp.last_match(1)

...
module WorkflowMgr
  ...
  class LSFBatchSystem < BatchSystem
    ...
    def vanquish_undead(bjobs, jobid)
      ...
      if !job[:reservation_time].nil? && !job[:lsf_runlimit].nil?
        ...
        if past > 10
          ...
          WorkflowMgr.stderr(
            "#{job[:jobid]}: Running for #{(reservation_age / 60).floor} of #{runlimit} reservation (past=#{past}).", 1
          )
        end
      end
      nil
    ...
    def unhold_job(joblist, jobid)
      ...
      _, _, exit_status = WorkflowMgr.run4(cmd, 30)
    ...
    def run_bhist_bjobs(nacctfiles = 1, bjobs: true)
      ...
      completed_jobs.split(/^-{10,}\n$/).each do |s|
        ...
        if /^\s*([0-9.]+) min of/.match(s)
          record[:lsf_runlimit] = ::Regexp.last_match(1).to_f
        end
        recordstring.gsub!(/\n\s{3,}/, '')
        recordstring.split(/\n+/).each do |event|
          case event.strip
          ...
          when /^Job *<(\d+)>, *(Job Name *<([^>]+)>,)? *User *<([^>]+)>,/
            ...
            record[:user] = ::Regexp.last_match(4)
            if event.strip =~ /Extsched <([^>]*)>/
              record[:extsched] = ::Regexp.last_match(1)
            end
          ...
          when /(\w+\s+\w+\s+\d+\s+\d+:\d+:\d+)(\s+\d\d\d\d)*: Submitted from host <[^>]+>, to Queue <([^>]+)>,/
            record[:submit_time] = lsf_time(::Regexp.last_match(1))
            record[:queue] = ::Regexp.last_match(2)
          when /(\w+\s+\w+\s+\d+\s+\d+:\d+:\d+)(\s+\d\d\d\d)*: Submitted from host <[^>]+>, CWD/
            record[:submit_time] = lsf_time(::Regexp.last_match(1))
          when /(\w+\s+\w+\s+\d+\s+\d+:\d+:\d+)(\s+\d\d\d\d)*: (Dispatched to|Started) /
            record[:start_time] = lsf_time(::Regexp.last_match(1))
          when /(\w+\s+\w+\s+\d+\s+\d+:\d+:\d+)(\s+\d\d\d\d)*: reservation_id *= *([0-9A-Za-z_.]+) *;/
            record[:reservation_id] = ::Regexp.last_match(3)
            record[:reservation_time] = lsf_time(::Regexp.last_match(1))
          ...
          when /(\w+\s+\w+\s+\d+\s+\d+:\d+:\d+)(\s+\d\d\d\d)*: Exited by LSF signal ([A-Za-z0-9_]+)/
            ...
            record[:native_state] = ::Regexp.last_match(3)
            record[:state] = "FAILED"

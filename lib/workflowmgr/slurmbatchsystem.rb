  ...
  class SLURMBatchSystem < BatchSystem
    ...
    def submit(task)
      ...
      task.attributes.each do |option, value|
        ...
        when :nodes
          ...
          if nodespecs.size > 1
            WorkflowMgr.stderr(
              "WARNING: Rocoto does not support Slurm's heterogeneous job feature.  However, Rocoto", 1
            )
            WorkflowMgr.stderr(
              "WARNING: does support Slurm requests for non-unifortm task geometries.  It does this by", 1
            )
            WorkflowMgr.stderr("WARNING: converting non-uniform requests into the smallest uniform task geometry", 1)
            WorkflowMgr.stderr(
              "WARNING: request that can accommodate the non-uniform one during batch job submission.", 1
            )
            WorkflowMgr.stderr("WARNING: It is up to the user to use the -m option of the srun command, or other", 1)
            WorkflowMgr.stderr(
              "WARNING: appropriate tools, in the job script to launch executables with an arbitrary", 1
            )
            ...
            WorkflowMgr.stderr(
              "WARNING: Rocoto has automatically converted '#{value}' to '#{nnodes}:ppn=#{maxppn}' for this job", 1
            )
            WorkflowMgr.stderr("WARNING: to facilitate the desired arbitrary task distribution.  Use", 1)
            WorkflowMgr.stderr(
              "WARNING: <nodes>#{nnodes}:ppn=#{maxppn}</nodes> in your workflow to eliminate this warning message.", 1
            )
          end
      ...
      if WorkflowMgr.dryrun_mode?
        ...
        WorkflowMgr.stderr(
          "Submitting #{task.attributes[:name]} using #{cmd} < #{tf.path} with input\n{{\n#{input}\n}}", 4
        )
      ...
      if output =~ /^Submitted batch job (\d+)/
        ...
        begin
          ...
          queued_jobs, = WorkflowMgr.run4(
            "squeue -u #{username} --federation -t all -O jobid:40,comment:32", @squeue_timeout
          )
        ...
        if String.method_defined? :encode
          queued_jobs = queued_jobs.encode('UTF-8', 'binary',
    ...
    def refresh_jobqueue(jobids)
      ...
      queued_jobs.split("\n").each do |job|
        ...
        record[:submit_time] = if job[110..139].strip =~ /^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d$/
                                 Time.local(*job[110..139].strip.split(/[-:T]/)).getgm
                               end

        # Extract the start time
        record[:start_time] = if job[140..169].strip =~ /^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d$/
                                Time.local(*job[140..169].strip.split(/[-:T]/)).getgm
                              end

        # Extract the end time
        record[:end_time] = if job[170..199].strip =~ /^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d$/
                              Time.local(*job[170..199].strip.split(/[-:T]/)).getgm
                            end
    ...
    def refresh_jobacct(delta_days)
      ...
      completed_jobs.split("\n").each do |job|
        ...
        # Extract the submit time
        record[:submit_time] = if jobfields[5] =~ /^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d$/
                                 Time.local(*jobfields[5].split(/[-:T]/)).getgm
                               end

        # Extract the start time
        record[:start_time] = if jobfields[6] =~ /^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d$/
                                Time.local(*jobfields[6].split(/[-:T]/)).getgm
                              end

        # Extract the end time
        record[:end_time] = if jobfields[7] =~ /^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d$/
                              Time.local(*jobfields[7].split(/[-:T]/)).getgm
                            end

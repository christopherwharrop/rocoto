  ...
  class PBSPROBatchSystem < BatchSystem
    ...
    def submit(task)
      ...
      task.attributes.each do |option, value|
        ...
        when :nodes
          ...
          value.strip.split("+").each do |chunk|
            mpiprocs = 1
            ompthreads = 1
            # ppn corresponds to mpiprocs
            if chunk =~ /ppn=(\d+)/
              mpiprocs = ::Regexp.last_match(1).to_i
            end
            # tpp corresponds to ompthreads per mpiproc
            if chunk =~ /tpp=(\d+)/
              ompthreads = ::Regexp.last_match(1).to_i
            end
      ...
      if WorkflowMgr.dryrun_mode?
        WorkflowMgr.stderr(
          "Dryrun Mode: Would submit #{task.attributes[:name]} using #{cmd} < #{tf.path} with input {{#{input}}}", 4
        )
    ...
    def refresh_jobacct(jobids)
      begin
        ...
        qstat, errors, exit_status = WorkflowMgr.run4(
